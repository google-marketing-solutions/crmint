...
class MlModelSingle(Resource):
  ...
  def get(self, id):  # pylint: disable=redefined-builtin
    ...
    m = models.MlModel.find(id)
    abort_when_not_found(m)
    return m
  ...
  def put(self, id):  # pylint: disable=redefined-builtin
    ...
    m.save()
    m.save_relations(args)

    pipelines = build_pipelines(m)
    m.save_relations({'pipelines': pipelines})
...
class MlModelList(Resource):
  ...
  def post(self):
    ...
    m = models.MlModel(name=args['name'])
    try:
      m.assign_attributes(args)
      m.save()
      m.save_relations(args)
