...
export class PipelineLogsComponent implements OnInit {
  ...
  loadLogs() {
    ...
    const promise1 =
        this.pipelinesService.getLogs(this.pipeline.id, this.getFilters())
            .then(logs => {
              if (!this.logs.length ||
                  logs.next_page_token !== this.filters.next_page_token) {
                this.logs =
                    plainToClass(Log, this.logs.concat(logs.entries) as Log[]);
                if (this.logs.length > 0) {
                  this.filters.next_page_token =
