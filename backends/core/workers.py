...
class MLOperationWaiter(MLWorker):
	""" Worker that checks an ML operation's status and respawns itslef until the operation is done."""

	def _execute(self):
	  self._get_ml_client()
	  request = self._ml_client.projects().operations().get(name=self._params['operation_name'])
	  operation = self.retry(request.execute)()
	  if operation['done'] != True:
	  	self._enqueue('MLOperationWaiter', {'operation_name': self._params['operation_name']}, 60) 
...
class MLTrainer(MLWorker):
  ...
  PARAMS = [
      ('project', 'string', True, '', 'ML project ID'),
      ('jobDir', 'string', True, '',
       'URI of folder to put output generated by AI platform (e.g. gs://bucket/folder)'),
      ('packageUris', 'string', True, '', 
        'URI of python package e.g. gs://bucket/folder/filename.tar.gz'),
      ('trainFile', 'string', True, '',
        'URI of file containing the training data e.g. gs://bucket/folder/filename.extension'),
      ('evalFile', 'string', True, '',
        'URI of file containing the evaluation data e.g. gs://bucket/folder/filename.extension'),
      ('scaleTier', 'string', True, '', 
        'Scale Tier e.g. BASIC, STANDARD_1'), 
      ('runtimeVersion', 'string', True, '', 
         'Runtime version e.g. 1.10'),
      ('pythonModule', 'string', True, '','Name of python module e.g. trainer.task'),
  ...
  def _execute(self):
    ...
    body = {
        ...
        'trainingInput': {
          'args': [
            '--train-files', 
            self._params['trainFile'], 
            '--eval-files', 
            self._params['evalFile'], 
            '--train-steps',
            str(self._params['trainSteps']), 
            '--eval-steps',
            str(self._params['evalSteps']),
            '--verbosity', 
            'DEBUG'
          ],
          ...
          'pythonModule': '%s.task' % (self._params['pythonModule'])
        }
    }
     
    project_id = 'projects/%s' % self._params['project']
    self._get_ml_client()
    request = self._ml_client.projects().jobs().create(parent=project_id, body=body)
    self.retry(request.execute)()
    job_name = '%s/jobs/%s' % (project_id, self._ml_job_id)
    self._enqueue('MLWaiter', {'job_name': job_name}, 60)
...
class MLVersionDeployer(MLWorker, StorageWorker):
  ...
  PARAMS = [
      ...
      ('versionName', 'string', True, '', 
        'Name of the version (Letters, numbers, underscores only. Must start with letter'),
      ('runtimeVersion', 'string', True, '', 
         'Runtime version e.g. 1.10'),
    ...
    for stat in stats:
    	if (stat.filename.find('saved_model.pb')) != -1:
    		if newestFile is None:
    			newestFile = stat
    	if newestFile:
    		if stat.st_ctime > newestFile.st_ctime:
    			newestFile = stat

    body = { 
    	"name":  self._params['versionName'],
    	"description": "Test from python",
    	"deploymentUri": "gs:/" + newestFile.filename[0:newestFile.filename.rfind('/')], 
