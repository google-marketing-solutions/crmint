...
# Nora edit    self._ml_client = build('ml', 'v1')
    credentials = ServiceAccountCredentials.from_json_keyfile_name(_KEY_FILE)
...
class MLTrainer(MLWorker):
  ...
  """ TODO(noralimbourg) insert dropdown to select scale-tier for distributed job"""
  """ TODO(noralimbourg) investigate if we want to expose options to select train-steps, verbosity etc"""
